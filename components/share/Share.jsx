'use client';
/* LIMITRA, product sharing (Web Share API + fallback modal + analytics) — ported from legacy/share.jsx */
...
export function trackShare(productId, source, event) {
  try {
...
export function ShareModal({ product, onClose }) {
  ...
  const channels = [
    ...
    { key: 'x', label: 'X', cls: 'x', glyph: 'X', href: `https://twitter.com/intent/tweet?text=${enc('Check out ' + product.name + ' on Limitra, ' + naira(product.price))}&url=${enc(url)}` },
...
export function ShareButton({ product, className, label = 'Share', compact }) {
