'use client';
/* LIMITRA, Video content system: hub, featured, reels feed, player, product videos — ported from legacy/videos.jsx */
...
import { trackShare } from '@/components/share/Share';
...
export const fmtViews = (n) => n >= 1000 ? (n / 1000).toFixed(n >= 10000 ? 0 : 1) + 'K' : '' + n;

/* ---- simulated player: poster + animated progress (no real media files) ---- */
export function VideoStage({ video, vertical, autoplay }) {
  ...
  return (
    <div className="vshare">
      {opts.map(([k, label, href, copyFirst]) => (
        <button key={k} className={'vshare-btn ' + k} title={'Share on ' + label} onClick={async () => { if (copyFirst) { try { await navigator.clipboard.writeText(url); toast('Link copied, paste it in ' + label); } catch (e) {} } window.open(href, '_blank', 'noopener'); trackShare(video.id, k, 'video-share'); }}>
...
export function ReelsFeed({ onClose, startId }) {
