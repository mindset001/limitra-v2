/* LIMITRA, shared Modal, AddressForm, CardForm (used by account + checkout) */
const { useState: useFState } = React;
const L = window.LIMITRA;

const NG_STATES = ['Lagos', 'Abuja (FCT)', 'Rivers', 'Oyo', 'Kano', 'Enugu', 'Kaduna', 'Delta'];
...
function FField({ label, value, onChange, err, placeholder, type = 'text', span }) {
  return (
...
function FSelect({ label, value, onChange, options, span }) {
  return (
    <div className="field" style={{ gridColumn: span === 'full' ? '1 / -1' : 'auto' }}>
      <label>{label}</label>
...
function AddressForm({ initial, onSave, onCancel }) {
  const [f, setF] = useFState(() => initial || { label: 'Home', name: L.USER.name, line: '', city: '', state: 'Lagos', phone: '', primary: false });
...
function CardForm({ initial, onSave, onCancel }) {
  ...
  const [f, setF] = useFState(() => initial || { name: L.USER.name, number: '', exp: '', cvv: '', primary: false });
  const [errs, setErrs] = useFState({});
  const set = (k, v) => setF(s => ({ ...s, [k]: v }));
  ...
  const submit = e => {
    ...
    if (editing) {
      ...
      onSave({ name: f.name, exp: f.exp, primary: f.primary, brand: L.cardBrand(digits), last: digits.slice(-4) });
    }
  };
  return (
    <form onSubmit={submit}>
...
function CardBrandMark({ brand, number }) {
  const b = brand || (window.LIMITRA.cardBrand ? window.LIMITRA.cardBrand(number) : 'Card');
...
function Dropdown({ value, options, onChange, icon, getIcon, getMeta, placeholder, width }) {
...
Object.assign(window, { Modal, AddressForm, CardForm, FField, FSelect, CardBrandMark, Dropdown, NG_STATES });
