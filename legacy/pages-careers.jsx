...
function ApplyJobForm({ job, onDone }) {
  ...
  const [f, setF] = useCState({ name:'', email:'', phone:'', link:'', note:'' });
...
function CareersPage() {
  ...
  const [dept, setDept] = useCState('All departments');
  const [type, setType] = useCState('All types');
  const [applyJob, setApplyJob] = useCState(null);

  const types = ['All types', 'Full-time', 'Part-time', 'Contract', 'Remote', 'Hybrid'];
  const counts = useCMemo(() => {
    const m = {}; JOBS.forEach(j => { m[j.dept] = (m[j.dept] || 0) + 1; }); return m;
  }, []);

  const results = useCMemo(() => {
