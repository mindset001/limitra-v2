...
function Countdown({ hours = 8 }) {
  const [t, setT] = useState(hours * 3600 + 1423);
  useEffect(() => {if (window.__STATIC_GALLERY) return;const id = setInterval(() => setT((x) => x > 0 ? x - 1 : 0), 1000);return () => clearInterval(id);}, []);
...
function HeroCarousel() {
  ...
  useEffect(() => {
    if (window.__STATIC_GALLERY || !t.heroAutoplay || paused) return;
...
function AffiliatePicks() {
  ...
  const picks = L.PRODUCTS.filter((p) => p.bestseller).slice(0, 4);
...
function FlashDeals() {
  const { go } = useStore();
  const deals = L.PRODUCTS.filter((p) => p.off > 0).slice(0, 6);
  return (
...
function Testimonials() {
  const items = L.REVIEWS.slice(0, 3);
  return (
    <section className="sec">
      <div className="wrap">
...
function HomePage() {
  useReveal();
  const trending = L.PRODUCTS.filter((p) => p.bestseller).slice(0, 5);
  const arrivals = L.PRODUCTS.filter((p) => p.badges.includes('new')).concat(L.PRODUCTS.slice(0, 5)).slice(0, 5);
  return (
      ...
      <div className="promote-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="promote-share">
          ...
          <button key={cls} className={'aff-share-btn ' + cls} title={'Share on ' + label} onClick={async () => {if (copyFirst) {try {await navigator.clipboard.writeText(url);toast('Link copied — paste it in ' + label);} catch (e) {}}window.open(href, '_blank', 'noopener,noreferrer');}}>
              {SM ? <SM name={label === 'X' ? 'X' : label} size={16} /> : <Icon name="share" size={16} />}
            </button>
          )}
        </div>
        <p className="promote-foot"><Icon name="info" size={13} /> Not an affiliate yet? <a onClick={() => {onClose();window.location.hash = '/affiliate';}}>Join the program</a> to start earning.</p>
      </div>
    </div>,
    document.body
  );
...
Object.assign(window, { HomePage, Countdown, HeroCarousel, HeroGrid, AffiliatePicks, ImageShowcase });