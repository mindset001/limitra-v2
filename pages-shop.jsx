...
function ShopPage({ cat, sub }) {
  const isDeals = cat === 'deals';
  const isNew = cat === 'new';
  const isTrending = cat === 'trending';
  const isAll = cat === 'all' || !cat;
  const isFashion = cat === 'fashion';
  const meta = L.CATEGORIES.find(c => c.slug === cat);
  let base = isDeals ? L.PRODUCTS.filter(p => p.off > 0) : isNew ? L.PRODUCTS.filter(p => (p.badges || []).includes('new')) : isTrending ? L.PRODUCTS.filter(p => p.bestseller) : isAll ? L.PRODUCTS : isFashion ? L.PRODUCTS.filter(p => p.category === 'womens' || p.category === 'mens') : L.byCat(cat);

  const subInfo = sub ? (window.SUBCAT_INDEX || {})[cat + '/' + sub] : null;
  if (subInfo) {
...
function CategoryBanner({ meta }) {
  const { go } = useStore();
  const subs = ((L.SUBCATS && L.SUBCATS[meta.slug]) || []).slice(0, 8);
  return (
      ...
      <div className={'cat-banner' + (meta.slug === 'adult' ? ' adult-pink' : '')} style={{ '--ph-b': meta.tint?.[1], '--ph-a': meta.tint?.[0] }}>
        <div>
          ...
          <div className="sub-chips">
            {subs.map(s => <button key={s} className="chip" onClick={() => go('shop', meta.slug, window.subSlugify ? window.subSlugify(s) : undefined)}>{s}</button>)}
          </div>
        </div>
...
function SearchPage({ q }) {
  ...
  const term = decodeURIComponent(q || query || 'all');
  const clean = term.toLowerCase().trim();
  const isBlank = !clean || clean === 'all';
  const results = useMemo(() => {
    if (isBlank) return L.PRODUCTS;
    return L.PRODUCTS.filter(p => (p.name + ' ' + p.brand + ' ' + p.category + ' ' + p.shot).toLowerCase().includes(clean));
  }, [clean]);
...
Object.assign(window, { ShopPage, SearchPage, ListCard, CategoryBanner });
