/* LIMITRA, Spin & Win signup reward wheel */
const { useState: useSpinState, useRef: useSpinRef, useEffect: useSpinEffect } = React;
const spinNaira = window.LIMITRA.naira;
...
function SpinWheel({ onClose }) {
  ...
  const [spinning, setSpinning] = useSpinState(false);
  const [rot, setRot] = useSpinState(0);
  const [result, setResult] = useSpinState(null);
      ...
      <div className="spin-modal" role="dialog" aria-label="Spin and win">
        ...
        {!result ? (
          ...
          <div className="spin-result">
            ...
            {result.kind === 'fixed' ? (
              <>
                <p className="sr-prize">You won <b>{spinNaira(result.value)}</b>. It’s been added to your points.</p>
                <div className="sr-code" style={{ textAlign: 'center' }}>
                  <small>Added to Lim Cash</small>
                  <div className="sr-code-row" style={{ justifyContent: 'center' }}><code>+{spinNaira(result.value)}</code></div>
                </div>
...
Object.assign(window, { SpinWheel, SPIN_PRIZES });
